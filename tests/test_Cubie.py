...
class TestSticker(unittest.TestCase):
    ...
    def test_eq(self):
        for c in self.allowed_chars:
            ...
            self.assertEqual(c >= s, c >= s.color)
            self.assertEqual(
                s < Cubie.Sticker(c.upper()), s.color < Cubie.Sticker(c.upper()).color
            )
            self.assertEqual(
                s <= Cubie.Sticker(c.upper()), s.color <= Cubie.Sticker(c.upper()).color
            )
    ...
    def test_init(self):
        ...
        for cubie in c.cubies:
            ...
            for face in cubie_list:
                ...
                self.assertEqual(
                    c.cubies[cubie].facings[face], Cubie.Cubie.facing_to_color(face)
                )
        ...
        for m, implications in Cubie.Cube.MOVES.items():
            ...
            for orig, dest in implications:
                ...
                for i in range(len(dest)):
                    self.assertEqual(
                        or_cubie.facings[orig[i]],
                        dest_cubie.facings[dest[i]],
                        msg="Fail in move %s" % m,
                    )
            ...
            for cubie in Cubie.Cube.CUBIES:
                if cubie not in moved_cubies:
                    ...
                    for i in range(len(cubie)):
                        self.assertEqual(
                            or_cubie.facings[cubie[i]], dest_cubie.facings[cubie[i]]
                        )
