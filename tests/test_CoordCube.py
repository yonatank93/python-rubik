    ...
    def _compare_lists(self, l1, l2):
        for i, _ in enumerate(l1):
            self.assertEqual(
                l1[i],
                l2[i],
                msg="Elements at index %d aren't the same %s != %s" % (i, l1[i], l2[i]),
            )
        ...
        for i, _ in enumerate(m1):
            for j, _ in enumerate(m1[i]):
                self.assertEqual(
                    m1[i][j],
                    m2[i][j],
                    msg="Elements at index [%d][%d] aren't the same %s != %s"
                    % (i, j, m1[i][j], m2[i][j]),
                )
