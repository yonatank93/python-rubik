...
class Sticker(object):
    ...
    def __init__(self, color):
        if color.lower() not in self.COLOURS:
            raise ValueError(
                "Color %s is not one of %s" % (color, ", ".join(self.COLOURS))
            )
    ...
    def __lt__(self, o):
        if isinstance(o, str):
            ...
            raise TypeError(
                "Don't know how to compare Sticker with %s" % o.__class__.__name__
            )
...
class Cubie(object):
    ...
    def __init__(self, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if key not in self.FACINGS:
                raise ValueError(
                    "Face %s is not one of %s" % (key, ", ".join(list(self.FACINGS)))
                )
    ...
    def __repr__(self):
        return "<%s: %s>" % (
            ...
            ", ".join(["%s: %s" % (k, v) for k, v in self.facings.items()]),
        )
...
class Cube(object):
    ...
    def move(self, move):
        ...
        for c_origin, c_dest in changes:
            ...
            origin_cubie = (
                original_cubies[c_t_origin]
                if c_t_origin in original_cubies
                else self.cubies[c_t_origin]
            )
    ...
    def search_by_colors(self, *args):
        ...
        for key, cubie in self.cubies.items():
            cubie_colors = tuple(
                sorted([str(c).upper() for c in cubie.facings.values()])
            )
