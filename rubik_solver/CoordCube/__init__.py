...
class CoordCube(object):
    ...
    def __init__(self, c):
        ...
        if not isinstance(c, CubieCube):
            raise ValueError(
                "c must be a CubieCube instance, got %s" % c.__class__.__name__
            )
    ...
    def move(self, m):
        ...
        if (
            self.URtoUL < 336 and self.UBtoDF < 336
...
def read_or_func_matrix(file_name, func):
    ...
    if os.path.exists(abspath):
        return [
            list(map(int, list(map(str.strip, l.split(","))))) for l in open(abspath)
        ]
    else:
        ret = func()
...
def build_twist_move():
    twist_move = [
        [0 for _ in range(CoordCube.N_MOVE)] for _ in range(CoordCube.N_TWIST)
    ]
    a = CubieCube()
...
def build_urf_to_dlf():
    urf_to_dlf = [
        [0 for _ in range(CoordCube.N_MOVE)] for _ in range(CoordCube.N_URFtoDLF)
    ]
    a = CubieCube()
...
def build_merge_ur_to_ul_and_ub_to_df():
    ...
    for uRtoUL in range(336):
        for uBtoDF in range(336):
            merge_ur_to_ul_and_ub_to_df[uRtoUL][uBtoDF] = CubieCube.getURtoDFs(
                uRtoUL, uBtoDF
            )
...
def build_slice_urf_to_dlf_parity_prun():
    slice_urf_to_dlf_parity_prun = [-1] * (
        CoordCube.N_SLICE2 * CoordCube.N_URFtoDLF * CoordCube.N_PARITY // 2
    )
                ...
                for j in [0, 1, 2, 4, 7, 9, 10, 11, 13, 16]:
                    ...
                    if (
                        CoordCube.getPruning(
                            ...
                            ((CoordCube.N_SLICE2 * newURFtoDLF) + newSlice) * 2
                            + newParity,
                        )
                        == 0x0F
                    ):
...
def build_slice_ur_to_df_parity_prun():
    slice_ur_to_df_parity_prun = [-1] * (
        CoordCube.N_SLICE2 * CoordCube.N_URtoDF * CoordCube.N_PARITY // 2
    )
        ...
        for i in range(CoordCube.N_SLICE1 * CoordCube.N_TWIST):
            ...
            if CoordCube.getPruning(slice_twist_prun, i) == depth:
                for j in range(18):
                    ...
                    if (
                        CoordCube.getPruning(
                            slice_twist_prun, CoordCube.N_SLICE1 * newTwist + newSlice
                        )
                        == 0x0F
                    ):
                        CoordCube.setPruning(
                            slice_twist_prun,
                            CoordCube.N_SLICE1 * newTwist + newSlice,
                            depth + 1,
                        )
                        done += 1
        depth += 1
        ...
        for i in range(CoordCube.N_SLICE1 * CoordCube.N_FLIP):
            ...
            if CoordCube.getPruning(slice_flip_prun, i) == depth:
                for j in range(18):
                    ...
                    if (
                        CoordCube.getPruning(
                            slice_flip_prun, CoordCube.N_SLICE1 * newFlip + newSlice
                        )
                        == 0x0F
                    ):
                        CoordCube.setPruning(
                            slice_flip_prun,
                            CoordCube.N_SLICE1 * newFlip + newSlice,
                            depth + 1,
                        )
                        done += 1
        depth += 1
...
CoordCube.MergeURtoULandUBtoDF = read_or_func_matrix(
    "merge_ur_to_ul_and_ub_to_df_move.csv", build_merge_ur_to_ul_and_ub_to_df
)

CoordCube.Slice_URFtoDLF_Parity_Prun = read_or_func_list(
    "slice_urf_to_dlf_parity_prun.csv", build_slice_urf_to_dlf_parity_prun
)

CoordCube.Slice_URtoDF_Parity_Prun = read_or_func_list(
    "slice_ur_to_df_parity_prun.csv", build_slice_ur_to_df_parity_prun
)

CoordCube.Slice_Twist_Prun = read_or_func_list(
    "slice_twist_prun.csv", build_slice_twist_prun
)

CoordCube.Slice_Flip_Prun = read_or_func_list(
    "slice_flip_prun.csv", build_slice_flip_prun
)
