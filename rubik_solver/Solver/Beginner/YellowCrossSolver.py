...
class YellowCrossSolver(Solver):
    ...
    def solution(self):
        ...
        up_yellows = [
            edge
            for edge in ["FU", "RU", "LU", "BU"]
            if self.cube.cubies[edge].color_facing("Y") == "U"
        ]
        ...
        if len(up_yellows) == 0:
            ...
            if not ("FU" in up_yellows and "BU" in up_yellows) and not (
                "RU" in up_yellows and "LU" in up_yellows
            ):
                # Rotate until L is at BU - LU
                while not (
                    self.cube.cubies["BU"].color_facing("Y") == "U"
            ...
            while not (
                self.cube.cubies["RU"].color_facing("Y") == "U"
                and self.cube.cubies["LU"].color_facing("Y") == "U"
            ):
                self.move("U", solution)
            # Line position
